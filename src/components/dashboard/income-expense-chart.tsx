...
export function IncomeExpenseChart({ transactions, month, year }: IncomeExpenseChartProps) {
  ...
  const data = useMemo(() => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      weeks.push({
        name: `${start}–${end}`,
        Receitas: weekTx.filter((t) => t.type === 'income').reduce((s, t) => s + t.amount, 0),
        Despesas: weekTx.filter((t) => t.type === 'expense').reduce((s, t) => s + t.amount, 0),
      })
    }
    return weeks
  ...
  return (
    <Card className="border-0 shadow-sm dark:bg-slate-900">
      <CardHeader className="pb-2">
        <CardTitle className="text-base font-semibold text-slate-900 dark:text-slate-100">Receitas vs Despesas</CardTitle>
      </CardHeader>
      <CardContent>
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={data} barSize={22} barGap={4}>
            ...
            />
            <Legend wrapperStyle={{ fontSize: '12px', paddingTop: '8px', color: tickColor }} />
